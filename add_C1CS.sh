...
--header 'api-version: v1' \
 | jq -r ".clusters[] | select(.name == \"${AZURE_PROJECT}\").id"`)

for i in "${!C1CSCLUSTERS[@]}"
do
  printf "%s\n" "C1CS: found cluster ${C1CSCLUSTERS[$i]}"
  if [[ "${C1CSCLUSTERS[$i]}" =~ "${AZURE_PROJECT}" ]]; 
  then
...
--header "api-secret-key: ${C1APIKEY}"  \
--header 'api-version: v1' \
 | jq -r ".scanners[] | select(.name == \"${AZURE_PROJECT}\").id"`)
...
--data-raw "{
    \"name\": \"${AZURE_PROJECT}\",
    \"description\": \"The SmartCheck scanner added by the CloudOneOnAWS project ${AZURE_PROJECT} \"
}" `
#echo $TEMPJSON | jq
...
--header "api-secret-key: ${C1APIKEY}"  \
--header 'api-version: v1' \
 | jq -r ".policies[] | select(.name == \"${AZURE_PROJECT}\").id"`)
...
--data-raw "{
    \"name\": \"${AZURE_PROJECT}\",
    \"description\": \"Policy created by the CloudOneOnAWS project ${AZURE_PROJECT}\",
    \"default\": {
        \"rules\": [
            {
                \"type\": \"registry\",
...
curl --request POST \
  ...
  --header "api-secret-key: ${C1APIKEY}" \
  --header 'content-type: application/json' \
  --data "{\"description\":\"AKS cluster added and Policy Assigned by the CloudOneOnAzure project ${AZURE_PROJECT}\",\"policyID\":\"${POLICYID}\"}" | jq
